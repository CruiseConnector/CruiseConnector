...
var TITEL = { ueberblick:'Überblick', nutzer:'Nutzer', fahrten:'Fahrten',
...
function bereichUeberblick(m){
  ...
  // 24 Stunden aus today - kam bisher nie zur Anzeige
  var t=DATEN.today;
  if(t && t.labels && t.rides){
    var fH=formNachWahl('heute');
    o+=block('Heute nach Stunden', dia({
      typ:fH.typ, flaeche:fH.flaeche, hoehe:170, id:0, abNull:true,
      reihen:[
    ...
    '<div class="legende"><span><i style="background:'+F.rot+'"></i>Fahrten</span><span><i style="background:'+F.lila+'"></i>Beiträge</span></div>',
    'Seit Mitternacht, Ortszeit Wien', umschalter('heute',true));
  }

  // 7 Tage gegen die 7 davor - kam bisher nie zur Anzeige
  var v=DATEN.compare;
  if(v && v.cur && v.prev){
    ...
    o+=block('Letzte 7 Tage gegen die 7 davor', vergleichHtml(felder, v.cur, v.prev)+
    ...
    'Jede Zeile an sich selbst gemessen, unabhängig vom Filter oben');
  }
