...
/* -- Wer kommt, wer bleibt, wer ist weg -----------------------------------
   2026-08-19 (vucko): "wie viele schon abgesprungen sind von der app oder sie
   deinstalliert haben?"

   EHRLICH GEKENNZEICHNET: Eine Deinstallation ist nicht messbar. Weder Apple
   noch Google melden sie, und eine geloeschte App kann sich nicht abmelden.
   Der beste Anhaltspunkt ist ein Geraet, das sich lange nicht gemeldet hat -
   das kann aber genauso ein ausgeschaltetes Handy sein. Deshalb steht dort
   "Geraet still" und nicht "deinstalliert". Hart gemessen ist nur, wer sich
   registriert und nie gefahren ist. */
function abwanderungBlock(){
  var a=DATEN.abwanderung;
  if(!a||!a.gesamt) return '';
  var nieProzent = Math.round(100*a.nie_gefahren/a.gesamt);
  var o='<div class="raster">'+
    karte('Registriert', z(a.gesamt))+
    karte('Nie gefahren', z(a.nie_gefahren),
      '<div class="fuss">'+nieProzent+'% aller Konten</div>')+
    karte('Nur eine Fahrt', z(a.eine_fahrt))+
    karte('7 Tage inaktiv', z(a.inaktiv_7d))+
  '</div>';

  o+='<div class="raster" style="margin-top:14px">'+
    karte('Ger&auml;t 7 Tage still', z(a.geraete_still_7d),
      '<div class="fuss">Hinweis, kein Beweis</div>')+
    karte('Ger&auml;t 30 Tage still', z(a.geraete_still_30d),
      '<div class="fuss">st&auml;rkster Deinstallations-Hinweis</div>')+
    karte('Ohne Ger&auml;t', z(a.ohne_geraet),
      '<div class="fuss">nie Push erlaubt</div>')+
    karte('30 Tage inaktiv', z(a.inaktiv_30d))+
  '</div>';

  var kl = nieProzent>80?'schlimm':(nieProzent>50?'':'gut');
  o+='<div class="hinweis '+kl+'"><b>'+z(a.nie_gefahren)+' von '+z(a.gesamt)+
     '</b> Konten haben noch keine einzige Fahrt gemacht. Nicht die '+
     'Deinstallationen sind das Thema, sondern der Weg bis zur ersten Fahrt. '+
     'Deinstallationen selbst sind technisch nicht messbar; die stillen '+
     'Ger&auml;te oben sind der beste verf&uuml;gbare Anhaltspunkt.</div>';

  var w=a.wochen||[];
  if(w.length){
    var zeilen='<div class="personen">';
    w.forEach(function(r){
      var farbe = r.prozent>=20?'var(--gut)':(r.prozent>=8?'var(--text)':'var(--schlimm)');
      zeilen+='<div class="person">'+
        '<span class="pname">ab '+esc(tagKurz(r.ab))+' &middot; '+z(r.neu)+' neu</span>'+
        '<span class="pwann"><b style="color:'+farbe+'">'+r.prozent+'%</b> gefahren'+
          ' &middot; '+z(r.noch_aktiv)+' noch aktiv</span>'+
      '</div>';
    });
    zeilen+='</div>';
    o+=block('Wochen-Kohorten', zeilen,
...
/* ── Routing ─────────────────────────────────────────────────────────── */
/* -- Wo Nutzer hinwollten, wo wir noch nicht abdecken ----------------------
   2026-08-18 (vucko): "sammel die meldungen im monitoring tool dann bitte."
   Quelle ist coverage_requests: jede Anfrage, die die Edge mit einer
   ehrlichen Meldung abgewiesen hat, statt den Nutzer zum Wiederholen
   aufzufordern. Vorher war das unsichtbar - in route_generation_events
   stehen keine Koordinaten, man wusste also nie, WO jemand angeklopft hat. */
function abdeckungswuenscheBlock(){
  var w=DATEN.abdeckungswuensche;
  if(!w) return '';
  if(!w.gesamt){
    return block('Wo Nutzer hinwollen',
      '<div class="hinweis gut">Niemand hat eine Route in einer Gegend angefragt, die wir nicht abdecken.</div>',
      'Anfragen ausserhalb des Liefergebiets');
  }

  var kopf='<div class="raster">'+
    karte('Anfragen gesamt', z(w.gesamt))+
    karte('Davon 7 Tage', z(w.woche))+
    karte('Betroffene Konten', z(w.personen),
      '<div class="fuss">ohne Anmeldung nicht zuordenbar</div>')+
    karte('Regionen', z(w.regionen.length))+
  '</div>';

  var zeilen='<div class="personen">';
  w.regionen.forEach(function(r){
    var kartenLink='https://www.openstreetmap.org/?mlat='+r.lat+'&mlon='+r.lng+'#map=9/'+r.lat+'/'+r.lng;
    var ort=naechsteStadt(r.lat,r.lng);
    var wo = ort
      ? esc(ort)+' <span style="opacity:.55">'+r.lat.toFixed(1)+'&deg;N '+r.lng.toFixed(1)+'&deg;O</span>'
      : r.lat.toFixed(1)+'&deg;N '+r.lng.toFixed(1)+'&deg;O'+(r.land?' ('+esc(r.land)+')':'');
    zeilen+='<div class="person">'+
      '<span class="pname"><a href="'+esc(kartenLink)+'" target="_blank" rel="noopener" '+
        'style="color:inherit;text-decoration:none">'+wo+'</a></span>'+
      '<span class="pwann"><b>'+z(r.anzahl)+'&times;</b> &middot; '+esc(tagKurz(r.zuletzt))+'</span>'+
    '</div>';
  });
  zeilen+='</div>';

  return kopf+block('Wo Nutzer hinwollen', zeilen,
    'Auf ein halbes Grad zusammengefasst, haeufigste zuerst. Klick oeffnet die Karte.');
}

/* Grobe Ortsangabe statt nackter Koordinaten. Bewusst eine kleine feste
   Liste: das Dashboard laedt nichts nach, auch keinen Geocoder. Trifft
   nichts zu, stehen die Koordinaten da - das ist ehrlicher als ein
   falscher Name. */
var STAEDTE=[
  ['Koeln',50.94,6.96],['Duesseldorf',51.23,6.78],['Dortmund',51.51,7.47],
  ['Essen',51.46,7.01],['Hannover',52.37,9.73],['Bremen',53.08,8.81],
  ['Hamburg',53.55,9.99],['Berlin',52.52,13.40],['Leipzig',51.34,12.37],
  ['Dresden',51.05,13.74],['Kassel',51.31,9.50],['Erfurt',50.98,11.03],
  ['Frankfurt',50.11,8.68],['Muenchen',48.14,11.58],['Stuttgart',48.78,9.18],
  ['Nuernberg',49.45,11.08],['Wien',48.21,16.37],['Graz',47.07,15.44],
  ['Salzburg',47.81,13.06],['Innsbruck',47.27,11.40],['Bregenz',47.50,9.75],
  ['Zuerich',47.38,8.54],['Bern',46.95,7.45],['Mailand',45.46,9.19],
  ['Amsterdam',52.37,4.90],['Bruessel',50.85,4.35],['Paris',48.86,2.35],
  ['Warschau',52.23,21.01],['Krakau',50.06,19.94],['Prag',50.08,14.44],
  ['Kopenhagen',55.68,12.57],['Madrid',40.42,-3.70],['Barcelona',41.39,2.17],
  ['Rom',41.90,12.50],['Lyon',45.76,4.84],['Athen',37.98,23.73],
  ['Istanbul',41.01,28.98],['Bukarest',44.43,26.10],['Budapest',47.50,19.04]
];
function naechsteStadt(lat,lng){
  var best=null, bestKm=1e9;
  for(var i=0;i<STAEDTE.length;i++){
    var dlat=(STAEDTE[i][1]-lat)*111;
    var dlng=(STAEDTE[i][2]-lng)*111*Math.cos(lat*Math.PI/180);
    var km=Math.sqrt(dlat*dlat+dlng*dlng);
    if(km<bestKm){ bestKm=km; best=STAEDTE[i][0]; }
  }
  return bestKm<=60 ? 'bei '+best : null;
}
