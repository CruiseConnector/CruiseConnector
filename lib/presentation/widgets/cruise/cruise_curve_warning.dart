...
class DetectedCurve {
  ...
  Color get color {
    switch (severity) {
      ...
      case CurveSeverity.sharp:
        return const Color(0xFFFFD54F); // gelb
      case CurveSeverity.hairpin:
        return AppAccentColors.accent; // rot
    }
  }
...
class CruiseCurveWarning extends StatelessWidget {
  const CruiseCurveWarning({super.key, required this.curve});
  ...
  static DetectedCurve? detectNextCurve({
    ...
    double cumDist = 0;
    final scanEnd = math.min(
      currentIndex + 400,
      coordinates.length - segmentStep,
    );
