          ...
          'steps': [
            {
              'maneuver': {
                'type': 'depart',
                'location': [11.57, 48.13],
              },
              'distance': 50.0,
              'name': 'Startstraße',
            },
            {'maneuver': maneuverData, 'distance': distance, 'name': stepName},
            ...extraSteps,
            {
              'maneuver': {
                'type': 'arrive',
                'location': [11.60, 48.15],
              },
              'distance': 0.0,
              'name': '',
            },
...
List<List<double>> _munichCoords() =>
    List.generate(30, (i) => [11.58 + i * 0.001, 48.14 + i * 0.0005]);
...
void main() {
  ...
  group('extractManeuvers – Abbiegemanöver', () {
    test('Links abbiegen → turn_left Icon', () {
      final data = _buildResponse(type: 'turn', modifier: 'left');
      final m = service
          .extractManeuvers(data, coords)
          .firstWhere(
            (x) => x.icon == Icons.turn_left,
            orElse: () => throw StateError('kein turn_left'),
          );
    ...
    test('Rechts abbiegen → turn_right Icon', () {
      ...
      final m = service
          .extractManeuvers(data, coords)
          .firstWhere(
            (x) => x.icon == Icons.turn_right,
            orElse: () => throw StateError('kein turn_right'),
          );
    ...
    test('Scharf links → turn_sharp_left Icon', () {
      ...
      final m = service
          .extractManeuvers(data, coords)
          .firstWhere(
            (x) => x.icon == Icons.turn_sharp_left,
            orElse: () => throw StateError('kein sharp_left'),
          );
    ...
    test('Scharf rechts → turn_sharp_right Icon', () {
      ...
      final m = service
          .extractManeuvers(data, coords)
          .firstWhere(
            (x) => x.icon == Icons.turn_sharp_right,
            orElse: () => throw StateError('kein sharp_right'),
          );
    ...
    test('Leicht links → turn_slight_left Icon', () {
      ...
      final m = service
          .extractManeuvers(data, coords)
          .firstWhere(
            (x) => x.icon == Icons.turn_slight_left,
            orElse: () => throw StateError('kein slight_left'),
          );
    ...
    test('Leicht rechts → turn_slight_right Icon', () {
      ...
      final m = service
          .extractManeuvers(data, coords)
          .firstWhere(
            (x) => x.icon == Icons.turn_slight_right,
            orElse: () => throw StateError('kein slight_right'),
          );
    ...
    test('U-Turn links → u_turn_left Icon', () {
      ...
      expect(
        all.any(
          (m) => m.icon == Icons.u_turn_left || m.icon == Icons.u_turn_right,
        ),
        isTrue,
      );
    });
  ...
  group('extractManeuvers – Spezialmanöver', () {
    test('Kreisverkehr → roundabout Icon + ManeuverType.roundabout', () {
      ...
      final m = service
          .extractManeuvers(data, coords)
          .firstWhere(
            (x) => x.maneuverType == ManeuverType.roundabout,
            orElse: () => throw StateError('kein roundabout'),
          );
    ...
    test(
      'Straßenende mit Straßenname → enthält Straßenname in Instruction',
      () {
        final data = _buildResponse(
          type: 'end of road',
          modifier: 'right',
    ...
    test('Autobahnauffahrt links → ramp_left Icon', () {
      ...
      final m = service
          .extractManeuvers(data, coords)
          .firstWhere(
            (x) => x.icon == Icons.ramp_left,
            orElse: () => throw StateError('kein ramp_left'),
          );
    ...
    test('Autobahnausfahrt rechts → ramp_right Icon', () {
      ...
      final m = service
          .extractManeuvers(data, coords)
          .firstWhere(
            (x) => x.icon == Icons.ramp_right,
            orElse: () => throw StateError('kein ramp_right'),
          );
    ...
    test('Fork links → fork_left Icon', () {
      ...
      final m = service
          .extractManeuvers(data, coords)
          .firstWhere(
            (x) => x.icon == Icons.fork_left,
            orElse: () => throw StateError('kein fork_left'),
          );
    ...
    test('Merge → merge Icon', () {
      ...
      final m = service
          .extractManeuvers(data, coords)
          .firstWhere(
            (x) => x.icon == Icons.merge,
            orElse: () => throw StateError('kein merge'),
          );
    ...
    test('continue geradeaus → straight Icon (ohne Richtungsänderung)', () {
      ...
      final m = service
          .extractManeuvers(data, coords)
          .firstWhere(
            (x) => x.icon == Icons.straight,
            orElse: () => throw StateError('kein straight'),
          );
