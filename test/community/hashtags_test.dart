...
void main() {
  group('1.3 — Client-Muster und Datenbank-Muster müssen dasselbe treffen', () {
    ...
    test(
      'die Migration benutzt genau das erwartete Muster — ändert es sich '
      'dort, wird dieser Test rot',
      () {
        expect(
          migration.contains(r"'#([[:alpha:]_][[:alnum:]_]{1,49})'"),
          isTrue,
          reason: 'Das Muster im Trigger post_hashtags_pflegen hat sich '
    ...
    test(
      'Umlaute sind ein echter Fall: #kurvenkönig muss anklickbar sein',
      () {
    ...
    test('mehrere Hashtags in einem Satz', () {
      expect(
        extractHashtags('Heute #kurven gefahren, danach #kaffee. Top!'),
        {'kurven', 'kaffee'},
      );
    });
    ...
    test(
      'Erwähnungen und Hashtags stören sich nicht — beide Muster laufen in '
      'derselben Schleife',
      () {
  ...
  group('1.3 — die Eingabe wird sauber gemacht, nicht gefaltet', () {
    ...
    test(
      'gefaltet wird hier NICHT — das macht die Datenbank, sonst laufen '
  ...
  group('1.3 — was im Code stehen muss', () {
    ...
    test(
      ...
      () {
        ...
        expect(service.contains("insert(") && service.contains('post_hashtags'),
            isFalse);
    ...
    test(
      'die Auslosungsliste ist nicht künstlich kurz — die RPC lässt bis 500 '
    ...
    test(
      'der Gruppen-Code liegt nach dem Umbau an Index 2 — ein übersehener '
    ...
    test('echte Umlaute in den Nutzertexten der neuen Seite', () {
      ...
      expect(seite.contains("'\$anzahl Beiträge'"), isTrue);
      expect(seite.contains("'\$anzahl Beitraege'"), isFalse);
